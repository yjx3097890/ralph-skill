...
├── config.example.yaml    # 配置示例（YAML 格式）
├── config.example.json    # 配置示例（JSON 格式）
├── src/ralph/             # 源代码
│   ├── core/             # 核心引擎
│   ├── models/           # 数据模型
...
- **config.example.yaml/json**: 配置示例文件，用户需要复制并修改为 `config.yaml` 或 `config.json`。
