...
## v1.2.0 (2024-03-06) - 端对端测试支持

### 新增功能

#### 1. 完整的端对端测试支持

**核心功能**：Ralph Skill 现在完全支持 E2E 测试，可以自动检测和执行 Playwright 或 Cypress 测试。

**主要特性**：

- **自动检测 E2E 框架**：根据配置文件中的 `e2e_runner` 字段自动检测
- **智能命令生成**：自动查找 `package.json` 中的测试脚本，支持自定义脚本名称
- **有序测试执行**：先执行单元测试，通过后再执行 E2E 测试
- **智能跳过**：单元测试失败时自动跳过 E2E 测试，节省时间
- **详细输出**：分别显示单元测试和 E2E 测试的结果

**新增方法**：

- `_get_test_commands()`: 返回所有测试类型的命令字典
- 改进 `_run_tests()`: 依次执行所有测试类型
- 保留 `_get_test_command()`: 用于向后兼容
...
- 新增 [端对端测试支持文档](./e2e_testing_support.md)
